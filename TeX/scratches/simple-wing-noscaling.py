...
def solve():
    ...
    ### Variables
    aspect_ratio = opti.variable(init_guess=10, scale=1)  # aspect ratio
    wing_area = opti.variable(init_guess=10, scale=1)  # total wing area [m^2]
    airspeed = opti.variable(init_guess=100, scale=1)  # cruising speed [m/s]
    weight = opti.variable(init_guess=10000, scale=1)  # total aircraft weight [N]
    CL = opti.variable(init_guess=1, scale=1)  # Lift coefficient of wing [-]
    ...
    return opti.solve(max_iter=100)
